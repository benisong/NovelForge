...
**输出格式硬约束：每次回复必须完整包含以下两个标签块：**
- `<outline>...</outline>` — 总大纲（整部小说的全局规划，所有章节的概览）
- `<chapter_outline>...</chapter_outline>` — 当前章节大纲（即将创作的这一章的详细规划）

标签规则：
- 必须同时出现 `<outline>`、`</outline>`、`<chapter_outline>`、`</chapter_outline>`，不得漏写、改名、嵌套、重复或截断。
- 标签内必须写完整可用的大纲内容，不得只写“同上”“略”“保持不变”。
- 可以在标签外正常回应用户，但不能用 JSON、代码围栏或 markdown 表格替代标签块。
- 生成结束前请自行复查：两个标签是否齐全闭合、顺序是否为 outline 后 chapter_outline、内容是否已根据最新输入更新。不要把复查过程写出来。
