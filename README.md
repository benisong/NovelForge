...
- 前端：统一使用移动端 Vue/Vite SPA（`static/mobile-app/`）作为主界面；旧桌面静态页仅保留为历史兼容壳
...
| 模块 | 作用 |
| --- | --- |
| Bot1 对话 | 普通章节规划对话；围绕当前章节推进、人物、冲突与节奏讨论 |
| Bot1 总纲工作流 | 独立的正式总纲设计 / 修正链路，采用“正式总纲 / 草稿总纲”分离与显式提交 |
...
| 多工作空间 | 每个朋友一个独立空间，主入口 URL 形如 `/m/w/<slug>/`，密码 + cookie 鉴权（`/w/<slug>/` 保留为兼容入口） |
| 移动端 | 作为统一主界面承载项目创建、总纲设计 / 修正、章节规划、创作、审核与记忆流程 |
...
```
http://你的服务器地址:17000/admin
# 首次打开是密码输入页，输 ADMIN_TOKEN 一次，cookie 记 30 天
```
...
把 `http://你的域名/m/w/alice/` 和密码发给本人即可。首次访问要求输入密码，输完会下发一个 30 天有效的 HttpOnly cookie，之后免登。

> `http://你的域名/w/alice/` 仍可访问，但现在只是兼容入口，会继续重定向到 `/m/w/alice/`。
> 当前统一入口会收口到移动端主界面：访问 `/` 或 `/w/<slug>/` 时，都会继续落到 `/m/w/<slug>/`。
> 只有 1 个工作空间时，首页 `/` 会直接进入该空间；有多个工作空间时，会先显示选择页。
...
┌─ 入口路由 (FastAPI) ─────────────────────────────────────┐
│  /                       picker（≥2 个工作空间时）         │
│  /admin                  管理员页（首次进要输 ADMIN_TOKEN）│
│  /admin/login            管理员登录页 / POST /admin/login  │
│  /admin/logout           管理员登出                        │
│  /w/{slug}/login         密码登录（5 失败/分钟会被锁 15 分钟） │
│  /w/{slug}/              兼容入口，统一重定向到移动端主界面     │
│  /m/                     移动端统一入口 / 多空间选择页            │
│  /m/w/{slug}/            移动端 SPA 主工作台                      │
│  /api/w/{slug}/...       受保护 API（require_workspace 依赖）│
│  /api/admin/workspaces   admin cookie 保护的 CRUD          │
...
| **`ADMIN_TOKEN`** | 空 | **必填**，管理员密码。首次打开 `/admin` 要求输入一次，之后 cookie 记 30 天。建议 ≥16 字符随机串（`openssl rand -hex 24`） |
...
新增/删除朋友只需访问 `/admin`（登过就免登）点按钮，**不用碰 Nginx 配置、不用 reload 服务**。
...
└─ static/
   ...
   ├─ index.html                 旧桌面兼容页（当前默认入口已统一收口到移动端）
   ├─ js/
   │  ├─ 00-api.js               旧桌面端 apiUrl + 401 自动跳登录
   │  └─ 01..10-*.js             旧桌面端历史业务模块
   └─ mobile-app/                Vue/Vite SPA（当前唯一主界面）
...
| POST | `/admin/login` | 管理员登录（Form 参数 `password=<ADMIN_TOKEN>`） |
| GET | `/admin/logout` | 管理员登出 |
| GET/POST/DELETE | `/api/admin/workspaces[/{slug}[/password\|rename]]` | 管理员 CRUD（cookie 保护） |
...
**Q：朋友丢了密码？**
进 `/admin`（如需登录就输 ADMIN_TOKEN），找到该 slug 点"改密码"。
...
以下内容已在架构讨论中确认有价值，但**留到后期再做**，本轮先不实现：

- **Bot1 最小调度器继续完善**
  - 已落地：最小调度状态字段、章节规划 / 总纲讨论摘要、短窗上下文、第一版近期提炼触发与辅助层抽离
  - 后续继续补的是真正独立的 `bot1_1 / bot4 / 4_1` 调度闭环，以及更成熟的阈值与阶段触发策略

- **bot1_1 / bot4 / 4_1 的完整调度编排**
  - 当前只明确了职责方向：Bot1 负责聊天，Bot1_1 偏近期规划，Bot4 / 4_1 偏长期总纲维护
  - 真正的多链路调度、触发顺序和后台任务机制留到后续阶段统一设计

- **Bot2 的“可见写作包”裁剪层**
  - 已明确总大纲原则上不能直接发给 Bot2，以避免剧透和全知视角污染
  - 但“哪些长期信息可下放给 Bot2、如何裁剪成当前章可见上下文”留待后续单独设计

- **移动端主界面后续体验打磨**
  - 当前已确定移动端 Vue SPA 为统一主界面，并已接入总纲设计 / 修正主流程
  - 后续继续补的是体验细节、残余旧口径清理，以及与新总纲工作流相关的边角交互

这些事项属于后续阶段的扩展任务，不影响本轮先落地：项目创建 → 进入总纲设计 → 草稿态编辑 → 提交/放弃。
